...
(function (wy) {
    ...
    var BaseViewManager = /** @class */ (function () {
        ...
        BaseViewManager.openPopUpView = function (disClass, effect, data, effectdata, graybg) {
            if (graybg === void 0) { graybg = true; }
            wy.Tween.switchPagePopType = effect;
            this.prePopUpView = this.nowPopUpView;
            var view = this.getView(disClass);
            view.show(data);
            view.x = wy.GameInterface.stage.stageWidth * 0.5 - view.width * 0.5 + view.anchorOffsetX;
            view.y = wy.GameInterface.stage.stageHeight * 0.5 - view.height * 0.5 + view.anchorOffsetY;
            if (graybg)
