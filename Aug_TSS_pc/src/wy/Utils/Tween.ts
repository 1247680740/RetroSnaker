				...
				if (name != 'notween') {
					...
					switch (this.switchPagePopType) {
						case wy.PopType.LEFTIN:
							...
							egret.Tween.get(child).wait((Math.random() * 0.5 + 0.5) * wy.PageSwitch.durationIn).to({ x: child.x}, wy.PageSwitch.durationIn + 500, egret.Ease.backOut).call((child, preTouchEnabled) => {
						...
						case wy.PopType.TOPIN:
							child.y -= dis;
							egret.Tween.get(child).wait((Math.random() * 0.5 +0.5) * wy.PageSwitch.durationIn).to({ y: child.y}, wy.PageSwitch.durationIn + 500, egret.Ease.backOut).call((child, preTouchEnabled) => {
								child.touchEnabled = preTouchEnabled;
							}, this, [child, preTouchEnabled]);
							break;
